...
- **Consistent setup across operating systems:** The Python wizard and launchers are designed for Windows, Linux and macOS, with credential-storage options suited to each environment. Native Windows and Linux validation is still pending.
...
2. Open the extracted `ehsan-multi-r-and-s-mcp` folder in Finder.
...
2. Open the extracted `ehsan-multi-r-and-s-mcp` folder.
...
The first run creates a private `.venv` folder and installs the required packages. The wizard then stores the device inventory in its `runtime` folder. The setup window is a terminal window; keep it open while answering prompts.
...
2. Right-click the ZIP, choose **Extract All**, and open the extracted `ehsan-multi-r-and-s-mcp` folder. Do not run files from inside the ZIP preview.
...
12. Save the file and restart Codex, or reload its MCP servers. Ask Codex **“List my enrolled devices using cisco-knight-multi-dev.”**

Do not replace Windows paths with macOS paths, do not type passwords into the Codex configuration, and do not edit `devices.json` or the credential files manually. If Codex reports that the server cannot start, open the Windows terminal in the extracted folder and run `setup.cmd` again; copy the complete error message before closing the window.
...
```text
cisco-knight-multi-dev — Device Setup
...
```sh
/Users/ehsan/Documents/MCP/ehsan-multi-r-and-s-mcp/.venv/bin/python \
  -m ehsan_mcp \
  --data-dir /Users/ehsan/Documents/MCP/ehsan-multi-r-and-s-mcp/runtime \
setup
```
...
- **Encrypted vault:** Scrypt derives a key from your master password; Fernet encrypts and authenticates the credential data. The master password is not saved. A terminal-launched server can use `serve --unlock-vault`. Desktop launchers need `EHSAN_MCP_MASTER_PASSWORD` injected through their secret-management mechanism; do not put it in the inventory or commit it.
...
```text
ehsan-mcp setup
ehsan-mcp client-config
ehsan-mcp serve
ehsan-mcp serve --unlock-vault
ehsan-mcp serve --prompt-secrets
```

For launchers, run the full virtual-environment Python path with `-m ehsan_mcp --data-dir <absolute-directory> serve`. Keep wizard prompts off the MCP stdio connection.
...
- Live read-only validation was performed against the existing lab switch. It required the legacy profile and negotiated an RSA host key, AES128-CBC and HMAC-SHA1; version and CDP reads succeeded. Native Windows/Linux execution and NX-OS/IOS-XR hardware validation are not yet completed.
